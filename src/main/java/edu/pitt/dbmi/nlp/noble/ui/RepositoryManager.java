...
public class RepositoryManager implements ActionListener, ListSelectionListener, PropertyChangeListener{
	...
	*/
	...
	private JComponent createToolBar(){
		...
		toolbar.add(createButton("COMPACT","/icons/Compact24.png"));
		toolbar.addSeparator();
	...
	private void doCompact(){
		int r = JOptionPane.showConfirmDialog(frame,"Are you sure you want to compact selected terminology?","Confirm",JOptionPane.YES_NO_OPTION);
		// if not canceled, remove entry
		if(r == JOptionPane.YES_OPTION){
			new Thread(new Runnable(){
				public void run(){
					...
					JDialog dialog = new JDialog(getFrame(), "Terminology Compact Progress",false);
					...
					for(Object o : terminologies.getSelectedValues()){
						...
						if(term instanceof NobleCoderTerminology){
							try {
								ConceptImporter.getInstance().compact((NobleCoderTerminology)term);
							} catch (IOException e) {
